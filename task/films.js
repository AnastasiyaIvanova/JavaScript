...
//первое задание
  ...
  films.forEach(function(item, i, films){
    if (item.director.oscarsCount == 0){
      countOfActors += item.actors.length; //подсчет кол-ва актеров
      const result = item.actors.reduce(function(sum, current){
        return sum + current.age;//сумма возрастов
      }, 0);
      sumAge += result;
    }
  });
    console.log(Math.round(sumAge / countOfActors));
